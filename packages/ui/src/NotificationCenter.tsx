...
export type NotificationVariant = 'info' | 'success' | 'warning' | 'error' | 'announcement' | 'promotion';

export type NotificationPriority = 'low' | 'medium' | 'high' | 'urgent';

export type NotificationStatus = 'unread' | 'read' | 'archived';

export type NotificationCategory = 'system' | 'user' | 'security' | 'feature' | 'marketing' | 'maintenance';
...
const variantToColors: Record<NotificationVariant, { 
  border: string; 
  bg: string; 
  text: string; 
  icon: string;
...
const priorityToColors: Record<NotificationPriority, { 
  color: string; 
  bg: string;
}> = {
  low: { color: 'var(--color-neutral-400)', bg: 'var(--color-neutral-800)' },
  medium: { color: 'var(--color-semantic-info)', bg: 'rgba(59, 130, 246, 0.1)' },
  high: { color: 'var(--color-semantic-warning)', bg: 'rgba(245, 158, 11, 0.1)' },
  urgent: { color: 'var(--color-semantic-error)', bg: 'rgba(239, 68, 68, 0.1)' }
};

const sizeToStyles: Record<'sm' | 'md' | 'lg', { 
  width: string; 
  maxHeight: string;
  padding: string;
  fontSize: string;
...
export const NotificationCenter: React.FC<NotificationCenterProps> = ({
  ...
  // Handle action click
  const handleActionClick = useCallback((action: NotificationAction, notification: Notification) => {
    if (!action.disabled) {
    ...
    return (
      <div
        ...
        style={{
          ...
          background: isUnread ? palette.bg : 'var(--color-background-secondary)',
          color: palette.text,
          cursor: 'pointer',
        ...
        onMouseEnter={(e) => {
          e.currentTarget.style.background = isUnread ? palette.bg : 'var(--color-neutral-800)';
        }}
        onMouseLeave={(e) => {
          e.currentTarget.style.background = isUnread ? palette.bg : 'var(--color-background-secondary)';
        }}
      >
  ...
  const renderEmptyState = () => (
    <div style={{
      ...
      {emptyIcon || (
        <Icon name="bell" size="2xl" style={{ marginBottom: '16px', opacity: 0.5 }} />
      )}
            ...
            <div key={groupName}>
              {groupByDate && groupName !== 'All' && (
                <div style={{
                  ...
                  marginBottom: '12px',
                  marginTop: groupName !== Object.keys(groupedNotifications)[0] ? '24px' : '0'
                }}>
                  {groupName}
                </div>
              )}
...
export const useNotifications = () => {
  ...
  const markAsRead = useCallback((id: string) => {
    setNotifications(prev => 
      prev.map(n => n.id === id ? { ...n, status: 'read' as NotificationStatus, readAt: new Date() } : n)
    );
  }, []);

  const markAllAsRead = useCallback(() => {
    setNotifications(prev => 
      prev.map(n => n.status === 'unread' ? { ...n, status: 'read' as NotificationStatus, readAt: new Date() } : n)
    );
  }, []);

  const archive = useCallback((id: string) => {
    setNotifications(prev => 
      prev.map(n => n.id === id ? { ...n, status: 'archived' as NotificationStatus, archivedAt: new Date() } : n)
    );
  }, []);

  const archiveAll = useCallback(() => {
    setNotifications(prev => 
      prev.map(n => n.status !== 'archived' ? { ...n, status: 'archived' as NotificationStatus, archivedAt: new Date() } : n)
    );
  }, []);
