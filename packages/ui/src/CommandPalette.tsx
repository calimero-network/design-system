...
export function CommandPalette({
  ...
  const [groupedItems, setGroupedItems] = useState<Record<string, CommandItem[]>>({});
  ...
  return (
    ...
    >
      {/* Search Input */}
      <div style={{ padding: '16px', borderBottom: `1px solid ${tokens.color.neutral[600].value}` }}>
      ...
      <div style={{ maxHeight, overflow: 'auto' }}>
        {filteredItems.length === 0 ? (
          <div style={{
            ...
            <Icon name="search" size="md" style={{ marginBottom: '12px', opacity: 0.5 }} />
      ...
      <div style={{
        ...
        <div>{filteredItems.length} command{filteredItems.length !== 1 ? 's' : ''}</div>
      </div>
    </div>
  );
}
