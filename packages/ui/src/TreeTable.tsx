...
export function TreeTable<T = any>({
  ...
  const filteredData = useMemo(() => {
    ...
    if (searchTerm) {
      result = result.filter((node) =>
        columns.some((column) => {
          const value = column.dataIndex ? (node.data as any)[column.dataIndex] : '';
  ...
  // Handle selection
  const handleSelectNode = useCallback((node: TreeNode<T>, checked: boolean) => {
              ...
              {columns
                ...
                .map(column => (
                  ...
                  >
                    ...
                    {Array.from(new Set(data.map(node => (node.data as any)[column.dataIndex as keyof T])))
                      .map(value => (
            ...
            <tr style={{ background: 'var(--color-background-secondary)' }}>
              {selectable && (
                <th style={{ 
                  ...
                  <input
                    type="checkbox"
                    checked={sortedData.length > 0 && sortedData.every(node => selectedKeys.includes(node.id))}
                    onChange={(e) => {
                      if (e.target.checked) {
                    ...
                    {selectable && (
                      <td style={{ 
                        ...
                        <input
                          type="checkbox"
                          checked={isSelected}
                          onChange={(e) => handleSelectNode(node, e.target.checked)}
                          style={{ margin: 0 }}
                        />
                      </td>
                    )}
                    {columns.map((column, colIndex) => (
                      <td
                        ...
                        style={{
                          ...
                          borderBottom: `1px solid ${tokens.color.neutral[700].value}`,
                          color: '#FFFFFF',
                          paddingLeft: colIndex === 0 ? `${(node.level || 0) * indentSize + 12}px` : currentSizeStyle.padding,
                        }}
                      >
                        {colIndex === 0 && hasChildren && (
                          ...
                          >
                            {isExpanded ? (
                              collapseIcon || <Icon name="chevron-down" size="sm" />
                            ) : (
                              expandIcon || <Icon name="chevron-right" size="sm" />
                            )}
                          </button>
                        )}
                        {colIndex === 0 && !hasChildren && (
                        ...
                        {column.render 
                          ? column.render((node.data as any)[column.dataIndex as keyof T], node, node.level || 0)
                          : String((node.data as any)[column.dataIndex as keyof T] || '')
                        }
                      </td>
                    ))}
                  </tr>
