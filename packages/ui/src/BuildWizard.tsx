...
export function BuildWizard({
  ...
  const validateCurrentStep = useCallback(async () => {
    ...
    try {
      ...
      if (!isValid) {
        setValidationErrors({ [currentStep.id]: 'Please complete all required fields' });
      }
      return isValid;
    } catch (error) {
  ...
  }, [validateCurrentStep, isLastStep, onComplete, data, steps, currentStepIndex, onStepChange]);
    ...
    >
      ...
      >
        <Box style={{ display: 'flex', alignItems: 'center', gap: '16px', marginBottom: '16px' }}>
          ...
          <Box style={{ flex: 1 }}>
            <Heading size="lg" style={{ margin: 0, color: 'var(--color-neutral-100)' }}>
              {currentStep.title}
            </Heading>
        ...
        {showProgress && (
          <Box>
            <Box style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '8px' }}>
              ...
              <Text size="sm" color="muted">{Math.round(progress)}%</Text>
            </Box>
...
export function TextInputStep({
  ...
  return (
    <Box>
      <Text size="md" weight="medium" style={{ marginBottom: '8px', color: 'var(--color-neutral-200)' }}>
        {label} {required && <span style={{ color: 'var(--color-semantic-error)' }}>*</span>}
      </Text>
      {multiline ? (
        <textarea
          value={value}
          onChange={(e) => updateData({ [label.toLowerCase().replace(/\s+/g, '_')]: e.target.value })}
...
export function CheckboxStep({
  ...
  return (
    <Box>
      <Text size="md" weight="medium" style={{ marginBottom: '16px', color: 'var(--color-neutral-200)' }}>
        {label} {required && <span style={{ color: 'var(--color-semantic-error)' }}>*</span>}
      </Text>
      <Box style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
        {options.map((option) => (
          ...
          >
            ...
            />
            <Box style={{ flex: 1 }}>
              <Text size="md" weight="medium" style={{ margin: 0, color: 'var(--color-neutral-100)' }}>
                {option.label}
              </Text>
              {option.description && (
