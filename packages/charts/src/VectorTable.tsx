      ...
      <table style={{ width: '100%', fontSize: '14px' }}>
        <thead>
          <tr style={{ textAlign: 'left', color: '#94a3b8' }}>
            <th style={{ padding: '8px 12px 8px 0', fontWeight: '500' }}>Name</th>
            {finalColumns.map((column, index) => (
              <th key={index} style={{ padding: '8px 12px', fontWeight: '500', textAlign: 'right' }}>
                {column.title}
              </th>
            ))}
          </tr>
          ...
          {data.map((row, index) => (
            <tr key={index} style={{ borderTop: '1px solid #374151' }}>
              <td style={{ padding: '8px 12px 8px 0', position: 'relative' }}>
                ...
                <span style={{ marginLeft: '16px', color: 'white' }}>{row.name}</span>
              </td>
              {finalColumns.map((column, colIndex) => {
                const key = column.title.toLowerCase().replace(/\s+/g, '_');
                const value = row[key] || 0;
                return (
                  <td key={colIndex} style={{ padding: '8px 12px', textAlign: 'right', color: 'white', fontVariantNumeric: 'tabular-nums' }}>
